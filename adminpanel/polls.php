			...
			<table class="table table-hover list_polls">
				...
				<tbody>
					<? while($poll = mysqli_fetch_array($query)){ 
						$i = $i + 1;
						echo "<tr><td>$i</td><td>$poll[name]</td><td>$poll[enable]</td><td><button class='glyphicon glyphicon-pencil edit-poll' aria-hidden='true' data-id='$poll[id]'></button></td><td><span class='glyphicon glyphicon-remove remove-poll' aria-hidden='true' data-id='$poll[id]'></span></td></tr>";
					}?>
				</tbody>
			</table>
		</div>
